...
fun LocationMarkerOverlay(
    ...
    /*
    Draws the current-location dot as a Compose overlay on top of the (non-Compose)
    Mapsforge MapView, since Mapsforge itself has no concept of a Compose-drawn marker.
    Recomputes its screen position whenever the map pans/zooms (via a Mapsforge Observer),
    and hides the marker for a short debounce window during active zoom/pinch gestures so
    it doesn't visibly lag behind the map underneath it.
    */
    ...
    fun recalculate() {
        /* Converts `position` to an on-screen pixel offset, relative to the map's current center/zoom. */
